    ...
    function AiSvgEdit(containerId, options = {}) {
        ...
        controls.innerHTML = `
            ...
            <div class="column">
                ...
                <button onclick="window.aiSvgEditor.savePNG()" title="儲存 PNG"><i class="fas fa-image"></i></button>
        ...
        this.savePNG = function() {
            ...
            if (saveOption === 'selected') {
                objectsToExport = activeObjects;
                console.log('選擇儲存選中物件為 PNG，數量:', activeObjects.length);
            } else {
                objectsToExport = canvas.getObjects().filter(obj => !obj.isGrid);
                console.log('選擇儲存整個畫布為 PNG，排除網格，物件數:', objectsToExport.length);
            }
            ...
            if (saveOption === 'selected') {
                canvas.getObjects().forEach(obj => {
                    if (!objectsToExport.includes(obj)) {
                        obj.visible = false;
            ...
            // 創建下載連結
            let downloadName = filename ? `${filename}.png` : (saveOption === 'selected' ? 'selected_drawing.png' : 'drawing.png');
            const a = document.createElement('a');
            a.href = pngData;
            a.download = downloadName;
            a.click();

            console.log(`PNG 已儲存，範圍: ${saveOption === 'selected' ? '選中物件' : '整個畫布'}，物件數: ${objectsToExport.length}，檔案名稱: ${downloadName}`);
        };
        ...
        document.getElementById('strokeColorDisplay').addEventListener('click', (e) => {
            ...
            if (!isVisible) {
                ...
                const canvasRect = canvasContainer.getBoundingRect();
